...
namespace U8_Sale
{
    public partial class frmSalePlan : UserControl
    {
        public frmSalePlan()
        {
            ...
            luecDepCode.Properties.DataSource = dschem.Tables[0];
...
ON s12.cSalePlanId = s.cSalePlanId";
        ...
        private void tsbDelete_Click(object sender, EventArgs e)
        {
            ...
            DBHelper.ExecuteNonQuery(DBHelper.ConnStrings, CommandType.Text, string.Format("delete from SL_P_SalePlan_Month where cSalePlanId='" + gdView.GetRowCellValue(iselectindex, "cSalePlanId").ToString() + "'"));
            DBHelper.ExecuteNonQuery(DBHelper.ConnStrings, CommandType.Text, string.Format("delete from SL_P_SalePlan where cSalePlanId='" + gdView.GetRowCellValue(iselectindex, "cSalePlanId").ToString() + "'"));
            DBHelper.ExecuteNonQuery(DBHelper.ConnStrings, CommandType.Text, string.Format("delete from SL_P_SalePlan_Shop where cSalePlanId='" + gdView.GetRowCellValue(iselectindex, "cSalePlanId").ToString() + "'"));
            DataBind();
        }
    }
}
